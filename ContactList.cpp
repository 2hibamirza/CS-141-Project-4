...
bool ContactList::operator==(const ContactList& otherList) const {
    if (contacts.size() != otherList.contacts.size()) {
        return false;
    }

    for (int i = 0; i < contacts.size(); i++) {
        if (contacts[i] != otherList.contacts[i]) {
            return false;
        }
    }
...
ContactList ContactList::operator+(const ContactList& otherList) const {
  ContactList resultList;
  
  for (Contact c : contacts) {
    resultList.addContact(c);
  }
  
  for (Contact c : otherList.contacts) {
    resultList.addContact(c);
  }
...
ostream& operator<<(ostream& os, const ContactList& cl) {
  os << "Contact List:" << endl;
  for (Contact c : cl.contacts) {
